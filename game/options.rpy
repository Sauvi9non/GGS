...
## 문자열을 _()로 둘러 쌓으면 씌우면 번역의 대상으로 표시됩니다.
    #게임 실행 시, 좌측 상단 프레임 게임 아이콘 우측에 표시되는 것 ggs_prototype0에서 광공 시뮬레이션으로 뜨도록 변경함.
define config.name = _("광공 시뮬레이터")
...
define build.name = "GGS"

#뒤로 스크롤(마우스 휠) 완전히 불능으로. (참고: https://www.reddit.com/r/RenPy/comments/k0ejsf/can_i_limit_the_scrollwheel/)
...
define config.main_menu_music = "audio/おしゃれなカフェ_일상.mp3"
...
#define config.save_directory = "ggs_prototype0-1625393392"
define config.save_directory = "GGS"
...
init python:
    ...
    build.classify('game/**.mp3', 'archive') #오디오파일
    build.classify('game/**.rpy', 'archive') #.RPY
    build.classify('game/**.rpyc', 'archive') #.RPYC
